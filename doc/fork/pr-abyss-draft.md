...
- **Challenge strategy** (per task config): `highest` (default, the highest unlocked stage) / `push` (hammer the first non-3-star stage) / `assign` (a manually assigned stage). `MaxRetry` caps rounds per stage per run and every retry swaps the node order of the two teams; when rounds are exhausted, `RetryExceeded` chooses between deferring to the next daily reset or waiting for next Monday.
- **Teams**: applies in-game preset teams (configurable preset index per node) with pixel verification and retry, since clicks during the node-focus animation can be swallowed. Per-node effects are equipped automatically: AS axioms take the first option, PF buffs take a configurable card (1/2/3/random, cards anchored by OCR since their heights vary with description text).
- **In-dungeon loop**: one state machine for all modes — map walking (joystick + A), battle detection via the wave flag, settlement exit. Auto battle and 2x speed are driven by the upstream `CombatState.handle_combat_state()` exactly like normal combat (it reads real button states and never mis-toggles, gated on a stable visible HUD); the abyss HUD auto-hides when idle, so a wake click fires only while it is hidden and auto/2x are unverified. Stall watchdog as backstop. AS additionally handles its mid-run settlement (go to node 2 / exit on failed node 1).
