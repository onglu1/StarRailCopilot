...
**⬜ 后续：自动配队/灵活配队**——现状两个固定预设序号对低练度号难满星。方案调研完成（同行项目/数据源/其他游戏先例/战斗模拟可行性），四档思路 A 标签弱点匹配 / B 统计驱动(含自己历史战报) / C 练度启发式 / D 模拟（判死刑）+ 三个快赢（失败换序重试、buff 关键词选择、星数感知换队），详见 [2026-06-12-team-comp-research.md](2026-06-12-team-comp-research.md)。等用户定方向（cookie/联网/组队范围/标签粒度四个决策点）。配队智能放 fork 层新文件，pr/abyss 保持纯净。
