...
**✅ 工具化定稿（2026-06-13 二轮反馈，commit 见 git log）**：三任务回**工具栏**（"放每周太怪"），**删除 Scheduler 组**——GUI 无 Enable/NextRun 只剩启动按钮，任务刷完即停（task_delay 调度尾巴移除）；删 RetryExceeded；换序改为 **SwapOnRetry 勾选项**（默认勾选，不勾=每轮保持原顺序）；预设编号支持 **1-12**（面板自由滚动：逐行拖拽+帧间位移实测定位 `abyss_panel_shift`，离线断言 166px/静止 0px，采样条带须取低位避免位移后出搜索窗）；PF 增益拆 **Node1Buff/Node2Buff**（各 1/2/3/random）。工具页白名单 [fork] 恢复。注意：工具模式下游戏崩溃→进程直接结束不自动重启（与 Daemon 同行为）；预设 4+ 滚动点击与增益卡 2/3 的 OCR 锚定点击未实机验证。
